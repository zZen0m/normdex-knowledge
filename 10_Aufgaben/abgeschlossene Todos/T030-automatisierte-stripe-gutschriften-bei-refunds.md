...
**Abschlussumfang:** `normdex-webapp-dev`, Branch `dev-server`, Commit `a8fd748`

## Abschluss und Abgrenzung

T030 ist mit dem vollständig implementierten, getesteten und auf dem Dev-System
bereitgestellten Stand abgeschlossen. Der Produktivrollout ist kein offenes
Umsetzungspaket von T030 mehr, sondern wird als eigener kontrollierter
Release- und Infrastrukturvorgang unter [[T031-webapp-produktivrelease-und-prod-stack-konsolidierung]]
geführt.
...
- [x] Dev-/Staging-Deployment mit Monitoring. *(Der Produktivrollout wird getrennt unter T031 durchgeführt.)*
...
- [x] Der für den Produktivrollout freigegebene Codepfad verhindert rechnungsbezogene Refunds ohne zugehörige Credit Note oder explizit dokumentierten Ausnahmegrund.
...
- 2026-06-21 (historischer Zwischenstand bei PAUSE, später vollständig erledigt): Paket E (E-Mail/Belegzustellung) vollständig und Paket D im Backend umgesetzt; Pause vor dem Paket-D-Frontend. Commit `e31af5f` (Branch `dev-server`), Backend Python-Syntax geprüft (py_compile OK), Tests/Frontend-UI zu diesem Zeitpunkt noch offen.
  ...
  - **Damals noch offen, anschließend in Commit `a8fd748` erledigt:** Paket-D-Frontend in `OrganizationCase.tsx` (Begriff „Gutschrift und Erstattung", erweiterte Vorschau mit Rechnung/Steuer/Beleg, Status-/Teilstatus- und PDF-Anzeige, Wiederholungs-Button, Warnungen bei Mehrdeutigkeit/`manual_review_required`); Unit-Tests für Paket E + Retry; vollständige Test-Suite; Migration gegen Dev-DB; danach Paket-D/E-Häkchen setzen und Container/Dev neu laden.
...
- 2026-06-22: T030 auf dem Dev-Stand abgeschlossen. `normdex-webapp-dev` steht auf `dev-server`/`a8fd748`, entspricht `origin/dev-server`, die Dev-API ist gesund und meldet Version `0.1.1`; die Dev-PostgreSQL-Datenbank steht auf Alembic-Head `e3c4d5e6f7a8`. Produktion wurde bewusst nicht verändert. Produktivrelease, Migration, Monitoring und Konsolidierung des doppelten Live-API-Compose-Betriebs werden getrennt unter T031 geführt.
- 2026-06-22: Nachtest-Nachbesserung auf Dev: Der Sofortkündigungs-Resolver filtert bezahlte Rechnungen jetzt vor der Mehrdeutigkeitsprüfung anhand der konkret ausgewählten Stripe-Subscription-Item-ID (Price-ID nur als Fallback). Beim gemeldeten Fall `sub_1Tg6qEF05ipkEAzmYUNmT0Uq` / `si_UkKptvigiv3NMO` wird dadurch eindeutig `NDX26-0530` statt der drei historischen Rechnungen gewählt. Zusätzlich wurde die Steueranzeige neutral auf „0 % USt. laut Originalrechnung“ formuliert und „Gutschriftbetrag (brutto)“ in Text- und HTML-Mail auf „Gutschriftbetrag“ gekürzt. 34 T030-Service-Tests, Frontend-Production-Build und der reale read-only Vorschaupfad sind grün; API und Frontend wurden neu auf Dev gebaut.
- 2026-06-22: Mehrfachauswahl fachlich geschärft: Lizenzen aus unterschiedlichen Originalrechnungen werden weiterhin bewusst nicht in eine einzelne Credit Note gepresst, da Stripe-Gutschriften rechnungsbezogen sind. Die Fehlermeldung nennt jetzt Rechnungsnummern und fordert zur getrennten Bearbeitung je Rechnung auf; die Dialogbeschreibung erklärt diese Grenze vorab. Die Vorschau zeigt zusätzlich den Stripe-Zahlungsstatus und den bereits bezahlten Betrag. Für `NDX26-0530` ist die sofortige Prorationsrechnung über 16,39 EUR als `paid` bestätigt; die nächste Stripe-Rechnung enthält regulär 49,00 EUR Hauptlizenz plus 29,00 EUR Zusatzlizenz. 35 T030-Service-Tests und Frontend-Production-Build grün, Dev neu gebaut und gesund.
- 2026-06-22: UI-Restfall korrigiert: Bei einer vollständig erstatteten Originalzahlung wird im Betragsfeld jetzt `0,00` statt des rein rechnerischen Aliquotwerts vorgeschlagen. Ein Wechsel der Lizenzauswahl verwirft außerdem die alte Vorschau und den alten Betrag, damit keine Werte einer vorherigen Auswahl weiterverwendet werden. Frontend erneut gebaut und auf Dev ausgerollt.
